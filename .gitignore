...
def gpu_loop(loc):
    root = tk.Tk()
    root.title("GPU Fence Stress Test: ")
    loc.acquire()
    try:
        tk.Label(root, text="GPU process has started:").grid(row=0)
    ...
    while True:
        start = timer()
        c = pow_gpu(a, b)
        m = "GPU is still running, the load loop took in {0} seconds".format(timer() - start)
        tk.Label(root, text=m).grid(row=0)
